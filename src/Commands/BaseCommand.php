...
class BaseCommand extends Command
{
    ...
    public function handle(CommandHistoryManagerInterface $history): void
    {
        ...
        if ($bool) {
            ...
            $history->saveToDriver($json, 'mesinhitung.log');
            // var_dump($history);
            $history->saveToDriver($json, 'latest.log');
        }
        ...
        if (count($data) > 10) {
            unset($data[0]);
            $this->clearAll('latest.log');
            foreach ($data as $value) {
                $history->saveToDriver(json_encode($value), 'latest.log');
            }
        }
    ...
    public function clearAll($name): void
    {
        file_put_contents('storage/' . $name, "");
    }
}
